...
def build_hdf(date, gps, temp, pres, mag, img):
    ...
    with h5py.File("test.hdf5", "w") as f:
        f.create_dataset("date", maxshape=(None,), dtype=h5py.string_dtype(), data=[date])
        f.create_dataset("gps", maxshape=(None,), dtype='f', data=[gps])
        f.create_dataset("temperature", maxshape=(None, 2), dtype='f', data=[temp])
        f.create_dataset("pressure", maxshape=(None,), dtype='f', data=[pres])
        f.create_dataset("magnetic field", maxshape=(None, 3), dtype='f', data=[mag])
        f.create_dataset("aurora img", maxshape=(None, 512, 512, 3), dtype='uint8', data=[img])
